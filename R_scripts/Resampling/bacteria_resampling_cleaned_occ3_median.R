...
rm(list=ls())
specseq=read.delim("bacteria_assigned_resampling_occ3.csv", header = TRUE, sep = ";")# data table was exported from xlsx here csv2: sep=";", dec=","
specseq <- specseq[,-1]
...
mean(sample_counts) # 90639.05
median(sample_counts) # 24285


### rarification
# resample loop for each sample/year present in the data table
nsampleff=24285# determine min. read counts for rarefaction, here automatic procedure to find the minimum within the data set
