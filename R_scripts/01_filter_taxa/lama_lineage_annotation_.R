...
selected.columns <- dplyr::select(bact_gen_spe_assign_clean, c("age", "Name_assigned", "TaxCount"))

write.csv2(selected.columns, "bacteria_age_count_clean.csv")

### make the data from long to wide
wide_bact <- pivot_wider(selected.columns, names_from = Name_assigned, names_sep = "_", values_from = TaxCount, values_fn = sum)
