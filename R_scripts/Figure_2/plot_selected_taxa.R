...
setwd("~/Postglacial-Bioweathering-main/R_scripts/Figure_1")
library(dplyr)
library(tidyr)
library(stringr)
library(ggplot2)
...
long_fungi_res_gg$age <- as.numeric(long_fungi_res_gg$X) / 1000
max.age <- round(max(long_fungi_res_gg$age))

### merge genera together
long_fungi_res_gg$Name_clean <- gsub("\\..*", "", long_fungi_res_gg$Name)
...
### extract only selected bacteria
...
long_bact_res_gg$age <- as.numeric(long_bact_res_gg$X) / 1000
max.age <- round(max(long_bact_res_gg$age))

### merge genera together
long_bact_res_gg$Name_clean <- gsub("\\..*", "", long_bact_res_gg$Name)   
...
max.age <- round(max(long_plant_res_gg$age)) 

### merge genera together
long_plant_res_gg$Name_clean <- gsub("\\..*", "", long_plant_res_gg$Name)       
...
### new order
new_order <- read.delim("ggplot_order_new.csv", sep = ";")

all_selected_gg_order <- merge(all_selected_gg, new_order, by = "Name_clean")

### force order
### it is a vector with unique taxa from plants to fungi to bacteria.
my_taxa_order=c(unique(plant_all_selected_input$Name_clean), unique(fungi_all_selected_input$Name_clean), unique(bact_all_selected_input$Name_clean))
...
all_gg <- ggplot(all_selected_gg_order, aes(x = merged_percent, y = age), fill = ecology) +
  ...
  scale_y_reverse(name = "Age (ka)", breaks = rev(seq(0, max.age, by = 5))) +
