...
rm(list=ls())
specseq=read.delim("bacteria_assigned_resampling_occ5.csv", header = TRUE, sep = ";")# data table was exported from xlsx here csv2: sep=";", dec=","
specseq <- specseq[,-1]
...
(nsampleff=sort(apply(t_specseq[,c(colstart:colend)], 2, sum))[1])# determine min. read counts for rarefaction, here automatic procedure to find the minimum within the data set
...
png(paste0("resampling/2023-03-07_bacteria_clean_occ5_resampled_totalspecies_Sampleeffort",nsampleff,"_plot.png"), width=480,height=480)
par(mar=c(8,4,3,1),las=2)
with(totspec,plot(Nspecies~T, main="number of species per sample"))
dev.off()

# save processed data
write.csv2(totspec, paste0("resampling/2023-03-07_bacteria_clean_occ5_resampled_resampled_totalspecies_Sampleeffort",nsampleff,".csv"), row.names=FALSE)	
write.csv2(totfam, paste0("resampling/2023-03-07_bacteria_clean_occ5_resampled_totalfamilies_Sampleeffort",nsampleff,".csv"), row.names=FALSE)	
...
pdf(paste0("resampling/2023-03-07_bacteria_clean_occ5_resampled_resampled_specieslevel_Sampleeffort",nsampleff,"_aggregated.pdf"))
par(mar=c(8,4,3,1),las=2)
...
# save processed data
write.csv2(speciesfamiliesdf_totfam, paste0("resampling/2023-03-07_bacteria_clean_occ5_resampled_resampled_specieslevel_Sampleeffort",nsampleff,"_aggregated.csv"), row.names=FALSE)
...
# export data
write.csv2(t(ordidf), paste0("resampling/2023-03-07_bacteria_clean_occ5_resampled_resampled_specieslevel_Sampleeffort",nsampleff,"_aggregated_pcainput.csv"))
...
png(paste0("resampling/2023-03-07_bacteria_clean_occ5_resampled_resampled_speciesnumber_Sampleeffort",nsampleff,"_aggregated_comparisonplot.png"), width=480,height=480)
par(mar=c(8,4,3,1),las=2)
...
png(paste0("resampling/2023-03-07_bacteria_clean_occ5_resampled_specieslevel_Sampleeffort",nsampleff,"_aggregated_pca_comparisonplot.png"), width=960,height=480)
par(mfrow=c(1,2))
